...
export const MantineTest = () => {
  ...
  const [inputSchools, setInputSchools] = useState('')

  const getSchools = async () => {
    const {data} = await axios.get(`${API_URL}${RESUME_URL.MASTER_SCHOOL}`, {params: {q: inputSchools}})
    console.log('inputSchools', inputSchools)
    return data.items
  }
  // queryKeys.master.schools.queryKey
  const {data: schools}: any = useFetch(['schools', inputSchools], getSchools, false, {suspense: false})
